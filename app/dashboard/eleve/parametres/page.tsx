...
import { Home, Users, BookOpen, Calendar, MessageCircle, CreditCard, Settings, LogOut, X, Eye, EyeOff, CheckCircle, AlertTriangle, Phone, Pencil, Camera } from "lucide-react";
...
function Toggle({ on, onChange }: { on: boolean; onChange: () => void }) {
  return (
    <button onClick={onChange} className="cursor-pointer shrink-0">
      <div className={`w-10 h-6 rounded-full flex items-center px-1 transition-colors ${on ? "bg-orange-500" : "bg-zinc-200"}`}>
...
export default function ParametresElevePage() {
  ...
  const [profilSucces, setProfilSucces] = useState(false);
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const url = await authService.uploadPhoto(file);
      setUtilisateur((prev) => prev ? { ...prev, photo_url: url } : prev);
    } catch (err) {
      setProfilErreur(err instanceof Error ? err.message : "Erreur lors de l'upload de la photo.");
      setPhotoPreview(null);
    } finally {
  ...
  async function modifierProfil(e: { preventDefault(): void }) {
    e.preventDefault();
    setProfilErreur(null);
    if (!editPrenom.trim() || !editNom.trim()) {
      setProfilErreur("Le prénom et le nom sont obligatoires.");
      return;
    }
    setProfilLoading(true);
    try {
      const u = await authService.mettreAJourProfil({
        prenom: editPrenom.trim(),
        nom: editNom.trim(),
        telephone: editTelephone.trim() || undefined,
      });
      setUtilisateur(u);
      setProfilSucces(true);
      setTimeout(() => { setProfilModal(false); setProfilSucces(false); }, 1800);
    } catch (err) {
      setProfilErreur(err instanceof Error ? err.message : "Erreur lors de la mise à jour.");
    } finally {
        ...
        <nav className="flex flex-col gap-1 p-2 flex-1">
          {NAV.map((item) => (
            <Link key={item.label} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${item.href === "/dashboard/eleve/parametres" ? "bg-orange-500 text-white" : "text-zinc-600 hover:bg-zinc-50 hover:text-zinc-800"}`}>
  ...
  const sections = [
    {
      title: "Notifications",
      items: [
        { label: "Nouveau cours planifié",      desc: "Quand un répétiteur planifie un cours avec vous", cle: "notif_nouveau_cours" as const },
        { label: "Rappels de cours",            desc: "Email 30 minutes avant chaque cours",             cle: "notif_rappel_cours" as const },
        { label: "Nouveaux messages",           desc: "Notification pour chaque message reçu",           cle: "notif_messages" as const },
        { label: "Confirmations de paiement",  desc: "Reçu après chaque transaction",                   cle: "notif_paiements" as const },
      ],
    },
  ];
    ...
    <div className="min-h-screen flex bg-zinc-100 font-sans">
        ...
        <main className="flex-1 p-6 flex flex-col gap-4 max-w-2xl">
          <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-zinc-800">Compte</h2>
              <button onClick={ouvrirProfilModal}
                className="flex items-center gap-1.5 text-xs font-semibold text-orange-500 hover:underline cursor-pointer">
                <Pencil size={12} />Modifier le profil
              </button>
            </div>
            <div className="flex items-center gap-4 mb-4 pb-4 border-b border-zinc-50">
              <div className="w-16 h-16 rounded-full bg-orange-100 border-2 border-orange-200 flex items-center justify-center shrink-0 overflow-hidden">
                {utilisateur?.photo_url ? (
                  <img src={utilisateur.photo_url} alt="avatar" className="w-full h-full object-cover" />
                ) : (
              ...
              <div>
                <p className="text-base font-bold text-zinc-800">{nomComplet}</p>
                <span className="text-xs font-semibold text-blue-500">Élève</span>
              </div>
            </div>
            <div className="flex flex-col divide-y divide-zinc-50">
              ...
              <div className="flex items-center justify-between py-2.5">
                ...
                <button onClick={() => setMdpModal(true)}
      ...
      {profilModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
              <h2 className="text-base font-extrabold text-zinc-800">Modifier le profil</h2>
              <button onClick={() => setProfilModal(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-zinc-100 text-zinc-400 cursor-pointer">
                <X size={16} />
              </button>
            </div>
            <form onSubmit={modifierProfil} className="p-5 flex flex-col gap-4">
              {profilSucces ? (
                <div className="flex flex-col items-center gap-3 py-4">
                  <CheckCircle size={32} className="text-green-500" />
                  <p className="text-sm font-semibold text-zinc-700">Profil mis à jour !</p>
                </div>
              ) : (
                <>
                  <div className="flex justify-center">
                    <button type="button" onClick={() => photoInputRef.current?.click()}
                      ...
                      <div className="w-20 h-20 rounded-full overflow-hidden bg-orange-100 border-2 border-orange-200 flex items-center justify-center">
                        {(photoPreview ?? utilisateur?.photo_url) ? (
                          <img src={photoPreview ?? utilisateur?.photo_url ?? ""} alt="photo" className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-2xl font-bold text-orange-600">{initiales}</span>
                        )}
                      </div>
                  ...
                  {profilErreur && (
                    <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl px-4 py-3">{profilErreur}</div>
                  )}
                  <div className="flex gap-3">
                    <div className="flex flex-col gap-1.5 flex-1">
                      <label className="text-sm font-semibold text-zinc-700">Prénom</label>
                      <input type="text" value={editPrenom} onChange={(e) => setEditPrenom(e.target.value)} required
                    ...
                    <div className="flex flex-col gap-1.5 flex-1">
                      <label className="text-sm font-semibold text-zinc-700">Nom</label>
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setProfilModal(false)}
                      className="flex-1 py-2.5 border border-zinc-200 text-zinc-600 font-semibold rounded-xl text-sm hover:bg-zinc-50 transition-colors cursor-pointer">
                      Annuler
                    </button>
